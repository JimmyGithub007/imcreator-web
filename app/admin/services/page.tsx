...
const Services = () => {
    ...
    const onSubmit: SubmitHandler<Inputs> = async (data) => {
        ...
        try {
            if (imageFile) {
                const imageRef = ref(sdb, `services/${imageFile.name}`);
    ...
    const handleConfirmDelete = async () => {
        if (deleteId) {
            const serviceDoc = doc(db, "services", deleteId);
    ...
    const handleOnDragEnd = async (result: any) => {
        ...
        for (const item of updatedRows) {
            const serviceDoc = doc(db, "services", item.id);
            await updateDoc(serviceDoc, { sortingId: item.sortingId });
        }
    };

    const getServices = async () => {
        const serviceQuery = await getDocs(query(collection(db, "services"), /*orderBy("sortingId")*/)); // updated
        const services = serviceQuery.docs.map(async (doc) => {
            const b = doc.data();
            return {
                id: doc.id,
    ...
    return (<main className={`flex flex-col items-center w-full`}>
        <motion.header
            ...
            <div className="flex items-center justify-between">
                <div className="text-lg">Admin / Services</div>
                ...
                <table className="table-auto">
                    <thead>
                        <tr>
                            ...
                            <th className="font-bold px-4 py-2 text-blue-950" align="left">Service name</th>
                            <th className="font-bold px-4 py-2 text-blue-950" align="center">Service image</th>
        ...
        >
            <div className="bg-[#f5f4f2] flex flex-col gap-6 items-center p-4">
                <h1>{editId ? "Edit" : "Create New"} Service</h1>
                    ...
                    <div id="uploadImageBox" onClick={() => {
                        ...
                        {selectedImage ? (
                            ...
                            <>
                                ...
                                <div className="text-center">Please upload service image</div>
                            </>
                        )}
