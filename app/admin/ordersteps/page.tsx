...
const OrderSteps = () => {
    ...
    const [rows, setRows] = useState<orderStep[]>([]);
    ...
    const onSubmit: SubmitHandler<Inputs> = async (data) => {
        ...
        try {
            if (imageFile) {
                const imageRef = ref(sdb, `orderSteps/${imageFile.name}`);
                ...
                uploadTask.on(
                    ...
                    async () => {
                        ...
                        if (editId) {
                            const orderStepDoc = doc(db, "orderSteps", editId);
                            await updateDoc(orderStepDoc, {
                                name: data.name,
                                statusId: Number(data.statusId),
                                imageUrl: imageUrl,
                            });
                        } else {
                            await addDoc(collection(db, "orderSteps"), {
                                name: data.name,
                                statusId: Number(data.statusId),
                                imageUrl: imageUrl,
    ...
    const handleConfirmDelete = async () => {
        if (deleteId) {
            const orderStepDoc = doc(db, "orderSteps", deleteId);
            await deleteDoc(orderStepDoc);
    ...
    const handleOnDragEnd = async (result: any) => {
        ...
        for (const item of updatedRows) {
            const orderStepDoc = doc(db, "orderSteps", item.id);
            await updateDoc(orderStepDoc, { sortingId: item.sortingId });
        }
    };

    const getOrderSteps = async () => {
        const orderStepQuery = await getDocs(query(collection(db, "orderSteps"), /*orderBy("sortingId")*/)); // updated
        const orderSteps = orderStepQuery.docs.map(async (doc) => {
            const b = doc.data();
            return {
                id: doc.id,
        ...
        const resolvedOrderSteps = await Promise.all(orderSteps);
        setRows(resolvedOrderSteps);
    };
    ...
    return (<main className={`flex flex-col items-center w-full`}>
        <motion.header
            ...
            <div className="flex items-center justify-between">
                <div className="text-lg">Admin / Order steps</div>
                ...
                <table className="table-auto">
                    <thead>
                        <tr>
                            ...
                            <th className="font-bold px-4 py-2 text-blue-950" align="left">Order steps name</th>
                            <th className="font-bold px-4 py-2 text-blue-950" align="center">Order steps image</th>
                    ...
                    <DragDropContext onDragEnd={handleOnDragEnd}>
                        <Droppable droppableId="orderSteps" type="group">
                            {(provided) => (
                                        ...
                                        <Draggable key={index} draggableId={index.toString()} index={index}>
                                            {(provided) => (
                                                <tr ref={provided.innerRef} {...provided.dragHandleProps} {...provided.draggableProps}>
                                                    ...
                                                    <td className="px-4 py-2 text-blue-950" align="center"><Image alt="" height={50} width={50} src={row.imageUrl} /></td>
        ...
        >
            <div className="bg-[#f5f4f2] flex flex-col gap-6 items-center p-4">
                <h1>{editId ? "Edit" : "Create New"} Order step</h1>
                    ...
                    <div id="uploadImageBox" onClick={() => {
                        ...
                        {selectedImage ? (
                            ...
                            <>
                                ...
                                <div className="text-center">Please upload order step image</div>
                            </>
                        )}
                        <input accept="image/*" ref={uploadRef} onChange={onFileChange} type="file" hidden />
                    </div>
                    <div className="flex h-2 items-center">{ !!errors.imageUrl && <span className="text-red-600 text-xs">*{errors.imageUrl?.message}</span> }</div>
                    <TextField className="w-full" label="Name" error={!!errors.name} helperText={errors.name?.message}
                        {...register("name",
                            { required: "Order step name is required", })
                        }
                    />
                    <Controller
                        name="statusId"
        ...
        >
            ...
            <DialogContent>
                <DialogContentText>
                    Are you sure you want to delete this order step?
                </DialogContentText>
            </DialogContent>
            <DialogActions>
